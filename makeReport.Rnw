...
DATA_THRU_WEEK <- 40 ## number from Thai filename 
transmittal_date <- "20141016" ## date data transmitted from thais
        
ANALYSIS_DATE <- "20141027" #as.character(Sys.Date(), "%Y%m%d")
DELIVERY_DATE <- "20141027" #as.character(Sys.Date(), "%Y%m%d") ## del date used in data pulls
...
make_country_prediction_line_graph(forecasts, counts, ylim_scale=1.7, show_unused_counts=FALSE)
@
