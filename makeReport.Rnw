...
ANALYSIS_DATE <- '20140926' #as.character(Sys.Date(), "%Y%m%d")
DELIVERY_DATE <- '20140926' #as.character(Sys.Date(), "%Y%m%d")
...
<<map1, fig.height=4, fig.width=2, dpi=400, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
...
<<map2, fig.height=4, fig.width=2, dpi=400, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
plot_forecast_map(forecast_data=forecasts, 
                  cdata=dat, 
                  include_legend=FALSE,
...
<<map3, fig.height=4, fig.width=4, dpi=400, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
plot_forecast_map(forecast_data=forecasts, 
                  cdata=dat, 
                  biweek=current_biweek+1, 
                  include_legend=TRUE,
...
\begin{figure}[H]
\caption{Outbreak probabilities across Thailand.}
\begin{tabular}{ccc}
...
(a) three biweeks ago & (b) one biweek ago & (c) one biweek ahead \\
\end{tabular}
\end{figure}
...
<<allDataCheck, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE>>=
data(thai_prov_data)
tpd <- thai_prov_data %>% 
        mutate(province=ISO,
