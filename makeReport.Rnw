...
<<installCruftery, echo=FALSE, message=FALSE, warning=FALSE>>=
library(devtools)
...
DATA_THRU_WEEK <- 36 ## number from Thai filename 
transmittal_date <- "20140929" ## date data transmitted from thais
        
ANALYSIS_DATE <- as.character(Sys.Date(), "%Y%m%d")
DELIVERY_DATE <- "20140929" #as.character(Sys.Date(), "%Y%m%d") ## del date used in data pulls
...
<<map1, fig.height=4, fig.width=2.5, dpi=200, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
plot_forecast_map(forecast_data=forecasts, 
                  cdata=dat, 
                  include_legend=FALSE,
                  biweek=current_biweek - 3, 
                  plot_type="outbreak")
@

<<map2, fig.height=4, fig.width=2.5, dpi=200, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
plot_forecast_map(forecast_data=forecasts, 
                  cdata=dat, 
                  include_legend=FALSE,
                  biweek=current_biweek - 1, 
                  plot_type="outbreak")
@

<<map3, fig.height=4, fig.width=5, dpi=200, eval=TRUE, echo=FALSE, message=FALSE, dev="png", include=FALSE, cache=TRUE, dev.args=list(bg='transparent')>>=
plot_forecast_map(forecast_data=forecasts, 
                  cdata=dat, 
                  biweek=current_biweek + 1, 
                  include_legend=TRUE,
...
\begin{figure}[H]
\caption{Probability of dengue fever outbreaks occuring by province in Thailand. Forecasts are made for biweekly periods. Dates in the figures indicate the first day of the forecasted biweek.  }
\begin{tabular}{lll}
  \includegraphics[width=.25\linewidth]{figure/map1.png} &   \includegraphics[width=.25\linewidth]{figure/map2.png} &   \includegraphics[width=.5\linewidth]{figure/map3.png}
\end{tabular}
\end{figure}
...
\begin{center}
\begin{tabular}{rl}
Data received through: & Week \Sexpr{DATA_THRU_WEEK}, \Sexpr{year(ymd(transmittal_date))} \\
